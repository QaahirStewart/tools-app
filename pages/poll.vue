    ...
    <div class="flex flex-col max-w-xl space-y-4">
      ...
      <div class="space-y-2">
        <div v-for="(option, index) in newPollOptions" :key="index">
          <button @click="selectedOption = option" :class="{ 'bg-green-500': selectedOption === option }"
            class="bg-black/5 hover:bg-black/10 rounded-lg flex justify-between items-center w-full">
            <div class="flex justify-between w-full px-6 py-4 relative">
              <div :style="{ width: option.percentage }" class="absolute left-0 top-0 bottom-0 bg-green-500 rounded-lg">
              </div>
              <h3>{{ option.name }}</h3>
      ...
      <div class="flex justify-between items-center">
        ...
        <div class="flex flex-wrap space-x-4">
          <button v-for="(platform, index) in platforms" :key="index" @click="selectedPlatform = platform.name"
            :class="{ 'border-black/25 border-2': selectedPlatform === platform.name }"
